...
w_mmd_pg = 0.00320;
w_mmd_alpha = 1000;
target_size = 80; % Number of samples used for mmd construction 
...
T1 = (tau_K_AkAl_tau(1:const.ngen,1:const.ngen));
T2 = (tau_K_AkAl_tau(const.ngen+1:end,const.ngen+1:end));
...
for j= 1:nt_r_x
[A1_test(:,:,j),Ao_test(:,j)] = dcopf_a1ao_dhatg(x_test (j,:)',data);
                   F_x_dc(:,j)   =  A1_test(:,:,j)*Res.Des_DCOPF.g + Ao_test(:,j);           
end
...
tau = 0.1;
% % % Constructing the two dataset given by the Ashley M Hou et.al 2020
for ii = 1:SA_DCOPF.target_size
A = randraw('uniform', [-1, 1], [length(const.loadbuses),length(const.loadbuses)]);
Abar = A*A';
for k = 1:length(const.loadbuses)
    for jj = 1:length(const.loadbuses)
        sig(k,jj,ii) = tau*(Abar(k,jj)/sqrt(Abar(k,k)*Abar(jj,jj)))*sqrt(const.d_hat(k)*const.d_hat(jj));
    end
end
xi(:,ii) = mvnrnd(zeros(length(const.loadbuses),1),sig(:,:,ii))';
end
