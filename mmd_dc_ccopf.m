...
[Des_DCOPF] = desired_dist_DCOPF_local(data,nt_r,frac_pen,dist_type,solver_name);
...
[~,xs] = xs_generate_DG(data,const.loadbuses,nt_r,length(const.loadbuses),rated_cap,dist_type);
...
nt_r_test =10000;
[~,x_test] = xs_generate_DG(data,const.loadbuses,nt_r_test,length(const.loadbuses),rated_cap,dist_type);
...
% tic
% [Res.SA_DC_CCOPF] = sample_approximation(data,Des_DCOPF,x_test,solver_name);
...
function[Des_DCOPF] = desired_dist_DCOPF_local(data,nt_r,frac_pen,dist_type,solver_name)
% data = case30;
% base =100;
const = ex_extract_ccDCOPF(data); %% Extract information from the MPC structure
...
function [SA_DC_CCOPF] = sample_approximation(data,Des_DCOPF,x_test,solver_name)

% solver_name = 'mosek';


[SA_DC_CCOPF] = desired_dist_DCOPF_sample(data,Des_DCOPF,x_test,solver_name);
...
function[SA_DCOPF] = desired_dist_DCOPF_sample(data,Des_DCOPF,x_test,solver_name)
% data = case30;
% base =100;
const = ex_extract_ccDCOPF(data); %% Extract information from the MPC structure
