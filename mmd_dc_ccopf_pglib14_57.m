...
for j= 1:target_size
[A1_rd_set(:,:,j),Ao_rd_set(:,j)] = dcopf_a1ao_dhatg(rd_set_full(j,:)',data);
end
t = size(A1_rd_set,1);
...
% [SA_DC_CCOPF] = sample_approximation(data,Des_DCOPF,solver_name);
...
for i = 1:D % D/2 becoz generating P and Q separately 
    switch dist_type
        case 'beta'
            xs_pen(:,i) = randraw('beta', [rated_cap*0.1 rated_cap 18 2], [1 nt_r])';
        case 'weibull'
            xs_pen(:,i) = randraw('weibull', [0 1/rated_cap rated_cap/2], [1 nt_r])';
        case 'laplace'
            xs_pen(:,i) = randraw('laplace',[ rated_cap*0.5 rated_cap*0.05], [1 nt_r])';
         case 'normal'
            xs_pen(:,i) = randraw('normal', [rated_cap/2 rated_cap/6], [1 nt_r])';
        case 'mix'
            xs_pen(:,i) = randraw('beta', [rated_cap*0.1 rated_cap 18 3], [1 nt_r])';
            % ----------------------
            xs_risk2(:,i) = randraw('laplace',[ rated_cap*0.5 rated_cap*0.05], [1 nt_r])';
...
function [SA_DC_CCOPF] = sample_approximation(data,Des_DCOPF,solver_name)
