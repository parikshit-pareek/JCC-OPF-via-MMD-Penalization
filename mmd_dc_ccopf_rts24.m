...
w_mmd_pg = 0.009; % 0.001 for normal, beta ,laplace  for mix 
...
nt_r = 300; % Total number of samples nt_r >>>>> N
...
% fname = sprintf('Res_24_rbus_%d_%d_%d.mat', rbus);
% save(fname,'Res');
...
const.Hr = const.H(:,rbus);
% const.g_u = const.g_u*0.8;
% const.f_u = const.f_u*0.8;
% const.f_l = const.f_l*0.8;
rated_cap = sum(data.bus(const.loadbuses,3)*frac_pen)/length(const.loadbuses); % Total Rated capacity is fraction of total load at load buses
% dist_type = 'beta';
